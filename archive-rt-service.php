...
if ( Opt::$layout == 'right-sidebar' || Opt::$layout == 'left-sidebar' ) {
	$post_classes = 'col-lg-6';
} else {
	$post_classes = 'col-xl-4 col-lg-4';
}

if( gardenar_option('rt_service_style') == 4 ) {
	if ( ( Opt::$layout == 'right-sidebar' || Opt::$layout == 'left-sidebar' ) ) {
		$post_classes = 'col-12';
	} else {
		$post_classes = 'dsad col-lg-6';
	}
}

if ( gardenar_option( 'rt_service_style' ) == 'default' ){
	$service_archive_layout 		= "rt-service-default rt-service-multi-layout-1";
} elseif ( gardenar_option( 'rt_service_style' ) == '2' ){
						...
						<div class="row g-4 item-parent rt-masonry-grid">
							<?php while ( have_posts() ) : the_post(); ?>
								<div class="<?php echo esc_attr( $post_classes );?> item rt-grid-item">
									<?php get_template_part( 'views/content-service', gardenar_option( 'rt_service_style' ) ); ?>
								</div>
							<?php endwhile; ?>
						</div>
					<?php else:?>
