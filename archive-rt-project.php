...
if( gardenar_option('rt_project_style') == 3 ) {
	if ( ( Opt::$layout == 'right-sidebar' || Opt::$layout == 'left-sidebar' ) ) {
		$post_classes = 'col-12';
	} else {
		$post_classes = 'col-12';
						...
						<div class="row g-4 item-parent">
							<?php while ( have_posts() ) : the_post(); ?>
								<div class="<?php echo esc_attr( $post_classes );?> item">
									<?php get_template_part( 'views/content-project', gardenar_option( 'rt_project_style' ) ); ?>
								</div>
							<?php endwhile; ?>
						</div>
					<?php else:?>
