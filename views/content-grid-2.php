		...
		<div class="post-thumbnail-wrap">
					...
					<div class="swiper-wrapper">
						<?php foreach( $rt_post_gallerys as $rt_posts_gallery ) { ?>
							<div class="swiper-slide">
								<?php echo wp_get_attachment_image( $rt_posts_gallery, 'gardenar-size3', "", array( "class" => "img-responsive" ) );
								?>
							</div>
						<?php } ?>
					</div>
			...
			<?php } else { ?>
				<figure class="post-thumbnail">
					<a class="post-thumb-link alignwide" href="<?php the_permalink(); ?>" aria-hidden="true" tabindex="-1"><?php the_post_thumbnail( 'gardenar-size3' ); ?></a>
				</figure><!-- .post-thumbnail -->
				<?php if ( gardenar_option( 'rt_blog_above_meta_visibility' ) && !empty( has_post_thumbnail() )) { ?>
					<div class="posted-on">
				...
				if ( gardenar_option( 'rt_video_visibility' ) == 1 && ( 'video' == get_post_format( get_the_ID() ) ) && !empty( $rt_youtube_link ) ) { ?>
