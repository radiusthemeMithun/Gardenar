;(function ($) {
    ...
    var Gardenar = {
        ...
        _init: function () {
            ...
            var offCanvas = {
                ...
                drawer: $('.gardenar-offcanvas-drawer'),
                drawerClass: '.gardenar-offcanvas-drawer',
                menuDropdown: $('.dropdown-menu.depth_0'),
            };

            Gardenar.menuDrawerOpen(offCanvas);
            Gardenar.offcanvasMenuToggle(offCanvas);
            Gardenar.headerSearchOpen();
            Gardenar.backToTop();
            Gardenar.counterUp();
            Gardenar.pricingTab();
            Gardenar.preLoader();
            ...
            Gardenar.headRoom();
            Gardenar.wow();
            Gardenar.rtElementorParallax();
            Gardenar.magnificPopup();
            Gardenar.imageFunction();
            Gardenar.hasAnimation();
            Gardenar.rtMasonary();
            Gardenar.swiperSlider($);
            Gardenar.horizontalSwiperSlider();
            Gardenar.heroSlider();
			Gardenar.ProgressBar();
			Gardenar.rtOpenTabs();
			Gardenar.rtBeer();
        },
        ...
        AjaxSearch: function () {
			...
			if ($("#rt_datafetch").length) {
				...
				$(document).on('gardenar_search_input_change', function () {
					fetchResults();
					$('#searchInput').focus();
				});
				function fetchResults() {
					...
					$.ajax({
						url: gardenarObj.ajaxURL,
						type: 'post',
						data: {
							action: 'rt_data_fetch',
							security: gardenarObj.gardenarNonce,
							keyword: keyword,
							meta: meta,
							searchkey: searchkey,
						},
				...
				$(".rt-addon-search .keyword").on("click", function () {
					...
					$(document).trigger('gardenar_search_input_change');
				});
    ...
    $(window).on('scroll', (event) => {
        Gardenar.backTopTopScroll(event);
    });
