...
class ServiceRelated {
	public static function rt_service_related() {
		...
		$related_item_number = gardenar_option( 'rt_service_related_limit' );
		...
		$query_type = gardenar_option( 'rt_service_related_query' );
		...
		if( gardenar_option( 'rt_service_related_sort' ) ){

			$post_order = gardenar_option( 'rt_service_related_sort' );
		...
		if( $related_query->have_posts() && gardenar_option( 'rt_service_related' ) ) { ?>
			<div class="rt-service-default rt-service-multi-layout-1 rt-related-service">
				<div class="container">
					<h2 class="related-title"><?php gardenar_html( gardenar_option( 'rt_service_related_title' ) , 'allow_title' );?></h2>
					<div class="row g-4 rt-masonry-grid">
						...
						$content = wp_trim_words( get_the_excerpt(), gardenar_option( 'rt_service_excerpt_limit' ), '' );
								...
								<div class="service-item <?php echo esc_attr( $icon_class ); ?>">
									...
									<div class="service-content">
										<div class="service-info">
											...
											<?php if ( gardenar_option( 'rt_service_ar_excerpt' ) ) { ?>
												<p><?php gardenar_html( $content , false ); ?></p>
											<?php } if ( gardenar_option( 'rt_service_read_more' ) ) { ?>
												<div class="rt-button">
													<a class="btn button-1" href="<?php the_permalink();?>">
														<?php esc_html_e('See Details' , 'gardenar' ); ?><i class="icon-rt-right-arrow-2"></i>
													</a>
												</div>
											<?php } ?>
										</div>
