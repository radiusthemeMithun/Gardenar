...
class Extras {
	...
	public function __construct() {
		...
		add_action( 'pre_get_posts', [ $this, 'gardenar_custom_pagesize' ],1 );
	...
	public function body_class( $classes ) {
		...
		$classes[] = 'gardenar-header-' . Opt::$header_style;
		$classes[] = 'gardenar-footer-' . Opt::$footer_style;
		...
		if ( gardenar_option( 'rt_tr_header_shadow' ) ) {
		...
		if ( gardenar_option( 'rt_sticy_header' ) ) {
			$classes[] = 'has-sticky-header';
		}

		if ( gardenar_option( 'rt_sticky_footer' ) ) {
			$classes[] = 'has-sticky-footer';
		}

		if ( gardenar_option( 'rt_blend' ) ) {
			$classes[] = 'rt-blend';
		}

		if ( gardenar_option( 'rt_remove_admin_bar' ) ) {
			$classes[] = 'remove-admin-bar';
		}

		if ( is_single() && Opt::$single_style ) {
			$classes[] = 'gardenar-single-' . Opt::$single_style;
		}
	...
	/*custom team archive */
	function gardenar_custom_pagesize( $query ) {
	...
	function menu_customize( $item_id, $item ) {
		// Mega menu
		$_mega_menu = get_post_meta( $item_id, 'gardenar_mega_menu', true );
		// Query string
		$menu_query_string = get_post_meta( $item_id, 'gardenar_menu_qs', true );
		?>
		...
		<p class="description widefat">
			<label class="widefat" for="gardenar-menu-qs-<?php echo esc_attr($item_id); ?>">
				<?php echo esc_html__( 'Query String', 'gardenar' ); ?><br>
				<input type="text"
					   class="widefat"
					   id="gardenar-menu-qs-<?php echo esc_attr($item_id); ?>"
					   name="gardenar-menu-qs[<?php echo esc_attr($item_id); ?>]"
	...
	function menu_update( $menu_id, $menu_item_db_id ) {
		$_mega_menu         = $_POST['gardenar_mega_menu'][ $menu_item_db_id ] ?? '';
		$query_string_value = $_POST['gardenar-menu-qs'][ $menu_item_db_id ] ?? '';

		update_post_meta( $menu_item_db_id, 'gardenar_mega_menu', $_mega_menu );
		update_post_meta( $menu_item_db_id, 'gardenar_menu_qs', $query_string_value );
	}
	...
	function menu_modify( $items, $menu, $args ) {
		foreach ( $items as $item ) {
			$menu_query_string = get_post_meta( $item->ID, 'gardenar_menu_qs', true );
	...
	public function search_form() {
		$output = '
		<form method="get" class="gardenar-search-form" action="' . esc_url( home_url( '/' ) ) . '">
            <div class="search-box">
				<input type="text" class="form-control" placeholder="' . esc_attr__( 'Search here...', 'gardenar' ) . '" value="' . get_search_query() . '" name="s" />
				<button class="item-btn" type="submit">
					' . gardenar_get_svg( 'search', false ) . '
					<span class="btn-label">' . esc_html__( 'Search', 'gardenar' ) . '</span>
				</button>
            </div>
		</form>
		';
