...
class DynamicStyles {
	...
	public function enqueue_scripts() {
		...
		wp_register_style( 'gardenar-dynamic', false, 'gardenar-main' );
		wp_enqueue_style( 'gardenar-dynamic' );
		wp_add_inline_style( 'gardenar-dynamic', $this->minify_css( $dynamic_css ) );
	}
	...
	private function inline_style() {
		...
		$primary_color   	= gardenar_option( 'rt_primary_color', '#01712D' );
		$secondary_color 	= gardenar_option( 'rt_secondary_color', '#FF9002' );
		$tertiary_color    	= gardenar_option( 'rt_tertiary_color', '#ffb000' );
		$body_bg_color   	= gardenar_option( 'rt_body_bg_color', '#ffffff' );
		$body_color      	= gardenar_option( 'rt_body_color', '#444444' );
		$border_color      	= gardenar_option( 'rt_border_color', '#E4E4E4' );
		$title_color     	= gardenar_option( 'rt_title_color', '#111111' );
		$button_color    	= gardenar_option( 'rt_button_color', '#01712D' );
		$button_text_color 	= gardenar_option( 'rt_button_text_color', '#01712D' );
		$black_color 		= gardenar_option( 'rt_button_text_color', '#111111' );
		$meta_color      	= gardenar_option( 'rt_meta_color', '#D5ED9E' );
		$meta_light      	= gardenar_option( 'rt_meta_light', '#F4FAEE' );
		$gray10          	= gardenar_option( 'rt_gray10_color', '#f1f1f1' );
		$gray20          	= gardenar_option( 'rt_gray20_color', '#E1EDE6' );
		...
		--rt-black-color: 		<?php echo esc_html( $black_color ); ?>;
		...
		--rt-container-width: 	<?php echo wp_kses_post( gardenar_option('container_width')); ?>px;
		}
	...
	protected function topbar_css() {
		$_topbar_active_color = gardenar_option( 'rt_topbar_active_color' );
		echo wp_kses_post( self::css( 'body .site-header .gardenar-topbar .topbar-container *:not(.dropdown-menu *)', 'color', 'rt_topbar_color' ) );
		echo wp_kses_post( self::css( 'body .site-header .gardenar-topbar .topbar-container svg:not(.dropdown-menu svg)', 'fill', 'rt_topbar_color' ) );

		if ( ! empty( $_topbar_active_color ) ) : ?>
			body .site-header .gardenar-topbar .topbar-container a:hover:not(.dropdown-menu a:hover),
			body .gardenar-topbar #topbar-menu ul ul li.current_page_item > a,
			body .gardenar-topbar #topbar-menu ul ul li.current-menu-ancestor > a,
			body .gardenar-topbar #topbar-menu ul.gardenar-topbar-menu > li.current-menu-item > a,
			body .gardenar-topbar #topbar-menu ul.gardenar-topbar-menu > li.current-menu-ancestor > a {
			color: <?php echo esc_attr( $_topbar_active_color ); ?>;
			}

			body .site-header .gardenar-topbar .topbar-container a:hover:not(.dropdown-menu a:hover svg) svg,
			body .gardenar-topbar #topbar-menu ul ul li.current-menu-ancestor > a svg,
			body .gardenar-topbar #topbar-menu ul.gardenar-topbar-menu > li.current-menu-item > a svg,
			body .gardenar-topbar #topbar-menu ul.gardenar-topbar-menu > li.current-menu-ancestor > a svg {
			fill: <?php echo esc_attr( $_topbar_active_color ); ?>;
			}
		<?php endif; ?>

		<?php
		echo wp_kses_post( self::css( 'body .gardenar-topbar', 'background-color', 'rt_topbar_bg_color' ) );
	...
	protected function header_css() {
		...
		$logo_dimension     = gardenar_option( 'rt_logo_width_height' );
		$logo_mobile_dimension     = gardenar_option( 'rt_mobile_logo_width_height' );
		$menu_border_bottom = gardenar_option( 'rt_menu_border_color' );
		...
		//Default Menu
		$_menu_color        = gardenar_option( 'rt_menu_color' );
		$_menu_active_color = gardenar_option( 'rt_menu_active_color' );
		$_menu_bg_color     = gardenar_option( 'rt_menu_bg_color' );
		$_sub_menu_bg_color     = gardenar_option( 'rt_sub_menu_bg_color' );

		//Transparent Menu
		$_tr_menu_color        = gardenar_option( 'rt_tr_menu_color' );
		$_tr_menu_active_color = gardenar_option( 'rt_tr_menu_active_color' );

		$_header_border     = gardenar_option( 'rt_header_border' );
		$_breadcrumb_border = gardenar_option( 'rt_breadcrumb_border' );
		$_preloader_bg_color = gardenar_option( 'preloader_bg_color' );
		?>

		<?php //Header Logo CSS ?>
		<?php if ( Opt::$header_width == 'full' ) :
			$h_width = '100%';
			if ( ( $header_width = gardenar_option( 'rt_header_max_width' ) ) > 768 ) {
		...
		<?php if ( ! empty( $_menu_color ) ) : ?>
			body .gardenar-navigation ul li a,
			body .gardenar-offcanvas-drawer ul.menu li a,
			body .gardenar-navigation ul li ul li a,
			body .menu-icon-wrapper .menu-search-bar {
				color: <?php echo esc_attr( $_menu_color ) ?>;
			}
			body .main-header-section svg,
			body .gardenar-navigation .caret svg {
		...
		<?php if ( ! empty( $_menu_active_color ) ) : ?>
			body .gardenar-navigation ul li a:hover,
			body .gardenar-navigation ul li.current-menu-item > a,
			body .gardenar-navigation ul li.current-menu-ancestor > a,
			body .gardenar-offcanvas-drawer ul li.current-menu-ancestor > a,
			body .gardenar-offcanvas-drawer ul.menu li a:hover,
			body .gardenar-navigation ul li ul li a:hover {
				color: <?php echo esc_attr( $_menu_active_color ) ?>;
			}
			body .gardenar-navigation ul li a:hover svg,
			body .gardenar-navigation ul li.current-menu-item > a svg,
			body .gardenar-navigation ul li.current-menu-ancestor > a svg {
		...
		<?php if ( ! empty( $_sub_menu_bg_color ) ) : ?>
			body .gardenar-navigation ul > li > ul,
			body .gardenar-navigation ul li.mega-menu > ul.dropdown-menu{
		...
		<?php if ( ! empty( $_tr_menu_color ) ) : ?>
			...
			body.has-trheader .site-header .gardenar-navigation *,
			body.has-trheader .site-header .gardenar-navigation ul li a {
			...
			body.has-trheader .site-header .gardenar-topbar .caret svg,
		...
		<?php if ( ! empty( $_tr_menu_active_color ) ) : ?>
			body.has-trheader .site-header .gardenar-navigation ul li a:hover,
			body.has-trheader .site-header .gardenar-navigation ul li.current-menu-item > a,
			body.has-trheader .site-header .gardenar-navigation ul li.current-menu-ancestor > a {
			...
			body.has-trheader .site-header .gardenar-navigation ul li.current-menu-ancestor > a svg,
			body.has-trheader .site-header .gardenar-navigation ul li.current-menu-item > a svg {
			fill: <?php echo esc_attr( $_tr_menu_active_color ); ?>
			}
		<?php endif; ?>
		<?php if ( ! empty( $menu_border_bottom ) ) : ?>
			body .gardenar-topbar,
			body .main-header-section,
			body .gardenar-breadcrumb-wrapper {
		...
		<?php if ( ! $_breadcrumb_border ) : ?>
			body .gardenar-breadcrumb-wrapper {border-bottom: none;}
		<?php endif; ?>
	...
	protected function breadcrumb_css() {
		$breadcrumb_color          = gardenar_option( 'rt_breadcrumb_color' );
		$rt_breadcrumb_hover       = gardenar_option( 'rt_breadcrumb_hover' );
		$breadcrumb_active         = gardenar_option( 'rt_breadcrumb_active' );
		$rt_breadcrumb_title_color = gardenar_option( 'rt_breadcrumb_title_color' );
		$rt_banner_color_opacity = gardenar_option( 'rt_banner_color_opacity' );

		$rt_banner_padding_top = gardenar_option( 'rt_banner_padding_top' );
		$rt_banner_padding_bottom = gardenar_option( 'rt_banner_padding_bottom' );

		$rt_banner_overlay1_color = gardenar_option( 'rt_banner_overlay1_color' );
		$rt_banner_overlay2_color = gardenar_option( 'rt_banner_overlay2_color' );

		if ( ! empty( $rt_breadcrumb_title_color ) ) { ?>
			.gardenar-breadcrumb-wrapper .entry-title {
				color: <?php echo esc_attr( $rt_breadcrumb_title_color ) ?> !important;
			}
		<?php }

		if ( ! empty( $breadcrumb_color ) ) { ?>
			.gardenar-breadcrumb-wrapper .breadcrumb a,
			.gardenar-breadcrumb-wrapper .entry-breadcrumb span a,
			.has-trheader .gardenar-breadcrumb-wrapper .breadcrumb a,
			.gardenar-breadcrumb-wrapper .entry-breadcrumb .dvdr {
				color: <?php echo esc_attr( $breadcrumb_color ) ?>;
			}
		<?php }

		if ( ! empty( $rt_breadcrumb_hover ) ) { ?>
			.gardenar-breadcrumb-wrapper .breadcrumb a:hover,
			.gardenar-breadcrumb-wrapper .entry-breadcrumb span a:hover {
			color: <?php echo esc_attr( $rt_breadcrumb_hover ) ?>;
			}
		<?php }

		if ( ! empty( $breadcrumb_active ) ) { ?>
			.gardenar-breadcrumb-wrapper .breadcrumb li.active .title,
			.gardenar-breadcrumb-wrapper .breadcrumb a:hover,
			.gardenar-breadcrumb-wrapper .entry-breadcrumb span a:hover,
			.gardenar-breadcrumb-wrapper .entry-breadcrumb .current-item,
			.has-trheader .gardenar-breadcrumb-wrapper .breadcrumb li.active .title,
			.has-trheader .gardenar-breadcrumb-wrapper .breadcrumb a:hover {
		...
		if ( ! empty( $rt_banner_overlay1_color && $rt_banner_overlay2_color ) ) { ?>
			.gardenar-breadcrumb-wrapper.has-bg::before {
	...
	protected function content_padding_css() {
		...
		protected function footer_css() {
			if ( gardenar_option( 'rt_footer_width' ) && gardenar_option( 'rt_footer_max_width' ) > 1400 ) {
			...
			echo wp_kses_post( self::css( 'body .site-footer .widget-title, .gardenar-footer-2 .site-footer .widget-title', 'color', 'rt_footer_widget_title_color' ) );
		...
		function site_fonts() {
			...
			$typo_body           = json_decode( gardenar_option( 'rt_body_typo' ), true );
			$typo_menu           = json_decode( gardenar_option( 'rt_menu_typo' ), true );
			$typo_heading        = json_decode( gardenar_option( 'rt_all_heading_typo' ), true );
			...
			echo wp_kses_post(self::font_css( '.gardenar-navigation ul li a', [
				'size'          => $typo_menu['size'],
			...
			foreach ( $heading_fonts as $heading ) {
				$font = json_decode( gardenar_option( "rt_heading_{$heading}_typo" ), true );
				if ( ! empty( $font['font'] ) ) {
		...
		static function css( $selector, $property, $theme_mod, $after_css = '' ) {
			$theme_mod = gardenar_option( $theme_mod );
