...
if ( ! function_exists( 'gardenar_custom_menu_cb' ) ) {
	/**
...
if ( ! function_exists( 'gardenar_menu_icons_group' ) ) {
	/**
	 * Get menu icon group
	 * @return void
	 */
	function gardenar_menu_icons_group( $args = [] ) {
		$default_args = [
			'hamburg'       => gardenar_option( 'rt_header_bar' ),
			'search'        => gardenar_option( 'rt_header_search' ),
			'login'         => gardenar_option( 'rt_header_login_link' ),
			'button'        => gardenar_option( 'rt_get_started_button' ),
			'button_label'  => gardenar_option( 'rt_get_started_label' ),
			'button_link'   => gardenar_option( 'rt_get_started_button_url' ),
			'has_separator' => gardenar_option( 'rt_header_separator' ),
			'compare'    	=> gardenar_option( 'rt_header_compare' ),
			'wishlist'    	=> gardenar_option( 'rt_header_wishlist' ),
			'add_to_cart' 	=> gardenar_option( 'rt_header_add_to_cart' )
		];
		...
		<div class="menu-icon-wrapper ml-auto">
			<ul class="menu-icon-action <?php echo esc_attr( $menu_classes ) ?>">
				<?php if ( $args['hamburg'] ) : ?>
					<?php gardenar_hanburger( 'desktop-hamburg' ); ?>
				<?php endif; ?>

				<?php gardenar_hanburger( 'mobile-hamburg' ); ?>
				...
				<?php if ( $has_button ) : ?>
					<li class="rt-button">
						<a class="btn button-2" href="<?php echo esc_url( $args['button_link'] ) ?>" aria-label="button link">
							<?php echo esc_html( $args['button_label'] ); ?><i class="icon-rt-right-arrow-2"></i>
						</a>
					</li>
				<?php endif; ?>
			</ul>
...
if ( ! function_exists( 'gardenar_require' ) ) {
	/**
	 ...
	 * @return false|void
	 */
	function gardenar_require( $filename, string $dir = 'inc' ) {
...
if ( ! function_exists( 'gardenar_readmore_text' ) ) {
	/**
...
if ( ! function_exists( 'gardenar_list_item_separator' ) ) {
	/**
	 ...
	 * @since 6.0.0
	 */
	function gardenar_list_item_separator() {
...
if ( ! function_exists( 'gardenar_posted_in' ) ) {
	/**
	 * Prints HTML with category list information about theme categories.
	 * @return string
	 */
	function gardenar_posted_in( $type = 'category' ) {
		$categories_list = get_the_category_list( gardenar_list_item_separator() );
		if ( 'tag' === $type ) {
			$categories_list = get_the_tag_list( '', gardenar_list_item_separator() );
		}
		if ( $categories_list ) {
			return sprintf(
...
if ( ! function_exists( 'gardenar_posted_on' ) ) {
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 * @return string
	 */
	function gardenar_posted_on() {
		$time_string = sprintf(
...
if ( ! function_exists( 'gardenar_get_svg' ) ) {
	/**
	 * Get svg icon
	 *
	 * @param $name
	 ...
	 * @return string|void
	 */
	function gardenar_get_svg( $name, $rotate = '' ) {
		$svg_list     = apply_filters( 'gardenar_svg_icon_list', [
...
if ( ! function_exists( 'gardenar_get_file' ) ) {
	/**
	 * Get File Path
	 *
	 * @param $path
	 ...
	 * @return string
	 */
	function gardenar_get_file( $path, $return_path = false ): string {
...
if ( ! function_exists( 'gardenar_get_img' ) ) {
	/**
	 * Get Image Path
	 *
	 * @param $filename
	 ...
	 * @return string|void
	 */
	function gardenar_get_img( $filename, $echo = false, $image_meta = '' ) {
		$path      = '/assets/images/' . $filename;
		$image_url = gardenar_get_file( $path );
...
if ( ! function_exists( 'gardenar_get_css' ) ) {
	/**
	 * Get CSS Path
	 *
	 * @param $filename
	 ...
	 * @return string
	 */
	function gardenar_get_css( $filename, $check_rtl = false ) {
		$min    = WP_DEBUG ? '.css' : '.css';
		$is_rtl = $check_rtl && is_rtl() ? 'css-rtl' : 'css';
		$path   = "/assets/$is_rtl/" . $filename . $min;

		return gardenar_get_file( $path );
	}
}

if ( ! function_exists( 'gardenar_get_js' ) ) {
	/**
	 * Get JS Path
	 *
	 * @param $filename
	 ...
	 * @return string
	 */
	function gardenar_get_js( $filename ) {
...
if ( ! function_exists( 'gardenar_option' ) ) {
	/**
	 ...
	 * @return mixed
	 */
	function gardenar_option( $key, $default = '', $return_array = false ) {
...
if ( ! function_exists( 'gardenar_get_social_html' ) ) {
	/**
	 * Get Social markup
	 *
	 * @param $color
	...
	function gardenar_get_social_html( $color = '' ) {
		ob_start();
...
if ( ! function_exists( 'gardenar_site_logo' ) ) {
	 ...
	 *
	 */
	function gardenar_site_logo( $with_logo = false, $custom_title = '', $mode = '' ) {
		$logo_main       = gardenar_option( 'rt_logo' );
		$logo_light      = gardenar_option( 'rt_logo_light' );
		$logo_mobile     = gardenar_option( 'rt_logo_mobile' );
...
if ( ! function_exists( 'gardenar_footer_logo' ) ) {
	/**
	 * Newfit Site Logo
	 *
	 */
	function gardenar_footer_logo() {
		$main_logo  = gardenar_option( 'rt_logo' );
		$logo_light = gardenar_option( 'rt_logo_light' );
		$site_logo  = $main_logo;
...
if ( ! function_exists( 'gardenar_scroll_top' ) ) {
	/**
	 * Back-to-top button
	 * @return void
	 */
	function gardenar_scroll_top( $class = '', $icon = 'scroll-top' ) {
		if ( gardenar_option( 'rt_back_to_top' ) ) {
			?>
			<a href="#" class="scrollToTop <?php echo esc_attr( $class ) ?>"><i class="icon-rt-right-arrow-2"></i></a>
			<?php
		}
	}
}

if ( ! function_exists( 'gardenar_meta_icons' ) ) {
	/**
	 * Get meta icons
	 *
	 * @param $name
	 ...
	 * @return string|void
	 */
	function gardenar_meta_icons( $name ) {
		if ( ! $name ) {
			return;
		}
		$icon_list = [
			...
			'category' => '<i class="icon-rt-category"></i>',
			'tag'      => '<i class="icon-tag"></i>',
			'reading'  => '<i class="icon-rt-clock"></i>',
			'views'    => '<i class="icon-rt-view"></i>',
		];
...
if ( ! function_exists( 'gardenar_reading_time' ) ) {
	/**
	 * Post reading time
	 */
...
if( !function_exists( 'rt_post_views' )){
	...
	function rt_post_views( $text = '', $post_id = 0 ){
		...
		if ( $view_count == 1 ) {
			$gardenar_view_html = esc_html__( 'View' , 'gardenar' );
		} else {
			$gardenar_view_html = esc_html__( 'Views' , 'gardenar' );
		}

		$gardenar_views_html = '<span class="view-number" >' . $view_count . '</span> ' . $gardenar_view_html;

		return '<span class="meta-views meta-item '. $views_class .'">'. $gardenar_views_html.'</span> ';
	}
}

if ( ! function_exists( 'gardenar_post_meta' ) ) {
	/**
	 * Get post meta
	 *
	 * @return string
	 */
	function gardenar_post_meta( $args ) {
		$default_args = [
			'with_list'     => true,
			'with_icon'     => true,
			'include'       => [],
			'class'         => '',
			'author_prefix' => __( 'By', 'gardenar' )
		];
		...
		$_meta_data['author']   = gardenar_posted_by( $args['author_prefix'] );
		$_meta_data['date']     = gardenar_posted_on();
		$_meta_data['category'] = gardenar_posted_in();
		$_meta_data['tag']      = gardenar_posted_in( 'tag' );
		$_meta_data['comment']  = esc_html( $comments_text );
		$_meta_data['reading']  = gardenar_reading_time();
		...
		foreach ( $meta_list as $key ) {
			...
			$output .= $args['with_icon'] ? gardenar_meta_icons( $key ) : null;
			$output .= $meta;
...
if ( ! function_exists( 'gardenar_post_thumbnail' ) ) {
	/**
	 * Displays post thumbnail.
	 * @return void
	 */
	function gardenar_post_thumbnail( $size = 'full', $thumb_date = false ) {
...
if ( ! function_exists( 'gardenar_post_single_thumbnail' ) ) {
	/**
	 * Display post details thumbnail
	 * @return void
	 */
	function gardenar_post_single_thumbnail( $size = 'full' ) {
		if ( ! Fns::can_show_post_thumbnail() ) {
			return;
		}
		?>
		...
		<?php } else { ?>
			...
			<?php if ( wp_get_attachment_caption( get_post_thumbnail_id() ) && gardenar_option( 'rt_single_caption_visibility' ) == 1 ) : ?>
				<figcaption class="wp-caption-text">
					<span><?php gardenar_html( wp_get_attachment_caption( get_post_thumbnail_id() ) ); ?></span>
				</figcaption>
			<?php endif; ?>
		<?php } ?>
		</div>
...
if ( ! function_exists( 'gardenar_entry_footer' ) ) {
	/**
	 * Gardenar Entry Footer
	 *
	 * @return void
	 *
	 */
	function gardenar_entry_footer() {
		if ( ! is_single() ) {
			if ( gardenar_option( 'rt_blog_footer_visibility' ) ) { ?>
			...
			if ( ( has_tag() && gardenar_option( 'rt_single_tag_visibility' ) ) || gardenar_option( 'rt_single_share_visibility' ) ) { ?>
				<footer class="entry-footer d-flex align-items-center justify-content-between">
					<?php if ( gardenar_option( 'rt_single_tag_visibility' ) ) { ?>
						<div class="post-tags">
							<?php if ( $tags_label = gardenar_option( 'rt_tags' ) ) {
								printf( "<span>%s</span>", esc_html( $tags_label ) );
							} ?>

							<?php
							gardenar_single_post_footer_meta(
								'content-below-meta', [ 'tag' ]
							);
							?>
						</div>
					<?php }
					if ( gardenar_option( 'rt_single_share_visibility' ) ) { ?>
						<div class="post-share">
							<?php if ( $tags_label = gardenar_option( 'rt_social' ) ) {
								printf( "<span>%s</span>", esc_html( $tags_label ) );
							} ?>
							<?php PostShare::gardenar_post_share(); ?>
						</div>
					<?php } ?>
				</footer>
				<?php
...
// single video
if ( ! function_exists( 'gardenar_post_single_video' ) ) {
	/**
	 * Display post details video
	 * @return void
	 */
	function gardenar_post_single_video() { ?>
		<?php $rt_youtube_link = get_post_meta( get_the_ID(), 'rt_youtube_link', true );
...
if ( ! function_exists( 'gardenar_entry_profile' ) ) {
	/**
	 * Gardenar Entry Profile
	 *
	 * @return void
	 *
	 */
	function gardenar_entry_profile() {
		if ( gardenar_option( 'rt_single_profile_visibility' ) ) {
					...
					<div class="profile-content">
						<div class="profile-author-info">
							...
							<div class="profile-info">
								...
								<?php if ( $prof_phone ) { ?>
									<span class="profile-phone"><?php echo esc_html__( 'Ph: ', 'gardenar' ) ?><?php echo esc_html( $prof_phone );?></span>
								<?php } ?>
							</div>
						</div>
						...
						<ul class="profile-author-social">
							...
							<?php if ( ! empty( $prof_tw ) ){ ?><li><a href="<?php echo esc_attr( $prof_tw ); ?>"><i class="icon-rt-twitter"></i></a></li><?php } ?>
							<?php if ( ! empty( $prof_lk ) ){ ?><li><a href="<?php echo esc_attr( $prof_lk ); ?>"><i class="icon-linkdin"></i></a></li><?php } ?>
...
if ( ! function_exists( 'gardenar_single_post_footer_meta' ) ) {
	/**
...
if ( ! function_exists( 'gardenar_entry_content' ) ) {
	/**
	 * Entry Content
	 * @return void
	 */
...
if ( ! function_exists( 'gardenar_sidebar' ) ) {
	/**
	 ...
	 * @return false|void
	 */
	function gardenar_sidebar( $sidebar_id ) {
		...
		?>
		<aside id="sidebar" class="gardenar-widget-area sidebar-sticky <?php echo esc_attr( $sidebar_cols ) ?>"
			   role="complementary">
...
if ( ! function_exists( 'gardenar_post_class' ) ) {
	/**
	 * Get dynamic article classes
	 * @return string
	 */
	function gardenar_post_class( $default_class = 'gardenar-post-card rt-grid-item' ) {
		$above_meta_style = 'above-' . gardenar_option( 'rt_single_above_meta_style' );


		$common_clsss = '';
		if ( is_single() ) {
			$common_clsss .= gardenar_option( 'rt_single_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = gardenar_option( 'rt_single_meta_style' );
			$post_classes = Fns::class_list( [ $common_clsss, $meta_style, $above_meta_style ] );
		} else {
			$common_clsss .= gardenar_option( 'rt_blog_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = gardenar_option( 'rt_blog_meta_style' );
			$blog_style   = 'blog-' . gardenar_option( 'rt_blog_style' );
...
if ( ! function_exists( 'gardenar_separate_meta' ) ) {
	/**
...
if ( ! function_exists( 'gardenar_single_entry_header' ) ) {
	/**
	 * Get above title meta
	 * @return string
	 */
	function gardenar_single_entry_header() {
		?>
...
if ( ! function_exists( 'gardenar_breadcrumb' ) ) {
	/**
	 * Gardenar breadcrumb
	 * @return void
	 */
	function gardenar_breadcrumb() {
		?>
		<nav aria-label="breadcrumb">
			<ul class="breadcrumb">
				<li class="breadcrumb-item">
					<a href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'Home', 'gardenar' ) ?></a>
					<span class="raquo"><i class="icon-rt-user-datalist-feature"></i></span>
				</li>
				<li class="breadcrumb-item active" aria-current="page">
					<?php
					if ( is_tag() ) {
						esc_html_e( 'Posts Tagged: ', 'gardenar' );
						?>
						...
						esc_html_e( 'Posts made in: ', 'gardenar' );
						echo esc_html( get_the_time( is_year() ? 'Y' : ( is_month() ? 'F, Y' : 'F jS, Y' ) ) );
						echo '</span>';
					} elseif ( is_search() ) {
						echo '<span class="title">';
						esc_html_e( 'Search results for: ', 'gardenar' );
						the_search_query();
						echo '</span>';
					} elseif ( is_404() ) {
						echo '<span class="title">';
						esc_html_e( '404', 'gardenar' );
						echo '</span>';
					} elseif ( is_single() ) {
						...
						echo '<span class="title">';
						esc_html_e( 'Blog', 'gardenar' );
						echo '</span>';
					} elseif ( class_exists( 'WooCommerce' ) and is_shop() ) {
						echo '<span class="title">';
						esc_html_e( 'Shop', 'gardenar' );
						echo '</span>';
					}
					?>
				</li>
...
if ( ! function_exists( 'gardenar_get_avatar_url' ) ) :
	function gardenar_get_avatar_url( $get_avatar ) {
...
function gardenar_comments_cbf( $comment, $args, $depth ) {
	...
	switch ( $comment->comment_type ) :
		...
		case 'trackback' : ?>
			<div class="pingback-entry"><span
					class="pingback-heading"><?php esc_html_e( 'Pingback:', 'gardenar' ); ?></span> <?php comment_author_link(); ?>
			</div>
			<?php
			break;
		default :
...
if ( ! function_exists( 'gardenar_hanburger' ) ) {
	 ...
	 * @return void
	 */
	function gardenar_hanburger( $class = '' ) {
		?>
		<li class="ham-burger <?php echo esc_attr( $class ) ?>">
			<button type="button" class="menu-bar trigger-off-canvas" aria-label="hamburger menu">
				<?php if( gardenar_option('rt_get_menu_label') ) { ?>
					<span class="menu-label"><?php echo esc_html( gardenar_option('rt_get_menu_label') ) ?></span>
				<?php } ?>
